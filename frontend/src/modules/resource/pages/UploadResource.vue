          ...
          <section v-if="currentStep === 1" key="step1" class="flex-1 bg-white rounded-3xl p-10 border border-gray-100 shadow-premium flex flex-col">
            <div class="flex-1 flex flex-col min-h-0">
              ...
              <div 
                ...
                :class="{ 'bg-trc/[0.05] border-trc/50': isDragOver }"
              >
                ...
                <input type="file" ref="fileInput" class="hidden" @change="handleFileUpload" accept=".pdf,.doc,.docx,.zip,.jpg,.jpeg,.png,.gif,.mp4,.avi,.mov" />
              </div>
            </div>
          ...
          <div class="relative z-10 flex-1 flex flex-col justify-center">
            <div v-if="formData.file" class="flex items-center gap-6 p-6 rounded-[2rem] bg-white shadow-sm border border-gray-50 group-hover:border-trc/20 transition-all">
              ...
              <div class="flex-1 min-w-0 pr-2">
                ...
                <span class="text-[0.65rem] font-bold text-gray-400 uppercase tracking-widest block">{{ (formData.file.size / (1024*1024)).toFixed(2) }} MB • Digital Asset Secured</span>
              </div>
            </div>
...
import { uploadResource, validateFile, type UploadMetadata } from '@/services/upload.service'
import {
  CheckOutlined,
  DownOutlined,
  CloseOutlined,
...
const nextStep = async () => {
   if (currentStep.value < steps.length) {
      ...
      // Final step - perform actual upload
      await performUpload()
   }
}
  ...
  switch (currentStep.value) {
    ...
    case 2:
      if (!formData.title.trim()) {
        message.error('Please enter a title for the resource')
        return false
      }
      if (!formData.abstract.trim()) {
        message.error('Please provide an abstract/summary')
        return false
      }
      break
    case 3:
      if (formData.authors.length === 0) {
        message.error('Please add at least one author/contributor')
        return false
      }
      break
  }
...
const performUpload = async () => {
  ...
  try {
    message.loading({ content: 'Securing archival node...', key: 'upload', duration: 0 })
    ...
    const response = await uploadResource(formData.file, metadata)

    message.success({ content: 'Digital node recorded successfully!', key: 'upload' })
    ...
    message.error({
      content: error?.response?.data?.message || error?.message || 'Upload failed. Please try again.',
      key: 'upload'
    })
  } finally {
...
const resetForm = () => {
  formData.file = null
  formData.title = ''
  formData.category = 'Historical Archives'
  formData.institution = 'Mekelle University'
  formData.abstract = ''
  formData.tags = ['AKSUM', 'HERITAGE']
  formData.authors = []
}
...
const handleAuthorSubmit = () => {
   if (newAuthor.name && newAuthor.role) {
      ...
      message.warning('Please fill in both name and role.')
   }
}
