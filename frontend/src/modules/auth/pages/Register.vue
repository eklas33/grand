          ...
          <form class="flex flex-col gap-5" @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-2">
                ...
                <BaseInput v-model="form.institution" placeholder="Mekelle University" :error="fieldErrors.institution" class="bg-[#F3F1FF] border-none shadow-none rounded-2xl overflow-hidden p-1" />
              </div>
              <div class="space-y-2 relative">
                ...
                <p v-if="fieldErrors.role" class="mt-2 text-xs font-semibold text-red-600">{{ fieldErrors.role }}</p>
              </div>
            </div>
            ...
            <!-- Confirm Password -->
            <div class="space-y-2">
              <label class="text-[0.9rem] font-bold text-gray-700">Confirm Password</label>
              <BaseInput v-model="form.confirm" type="password" placeholder="Confirm password" :error="fieldErrors.confirm" class="bg-[#F3F1FF] border-none shadow-none rounded-2xl overflow-hidden p-1" />
            </div>
            ...
            <p v-if="fieldErrors.agree" class="mt-2 text-xs font-semibold text-red-600">{{ fieldErrors.agree }}</p>
          ...
          <form class="flex flex-col gap-4" @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <p v-if="fieldErrors.institution" class="mt-2 text-xs font-semibold text-red-600">{{ fieldErrors.institution }}</p>
              </div>
              <div class="space-y-1">
    ...
    <div class="flex min-h-screen flex-col bg-white px-6 py-10 md:hidden">
       ...
       <div class="mx-auto w-full max-w-[420px] animate-fade-up">
          ...
          <form class="flex flex-col gap-5" @submit.prevent="handleSubmit">
             ...
             <!-- Institution & Role Grid -->
             <div class="grid grid-cols-2 gap-4">
               <BaseInput v-model="form.institution" label="Institution" placeholder="MU" :error="fieldErrors.institution" />
             ...
             <BaseInput v-model="form.password" label="Password" type="password" placeholder="••••••••" :error="fieldErrors.password" />
             <BaseInput v-model="form.confirm" label="Confirm Password" type="password" placeholder="••••••••" :error="fieldErrors.confirm" />
...
function validate(): boolean {
  ...
  if (!form.institution) {
    fieldErrors.value.institution = 'Please enter your institution.'
  }
  if (!form.role) {
  ...
  if (!form.confirm) {
    fieldErrors.value.confirm = 'Please confirm your password.'
  }
  if (form.password && form.confirm && form.password !== form.confirm) {
    fieldErrors.value.confirm = 'Passwords do not match.'
  }
  if (!form.agree) {
    fieldErrors.value.agree = 'You must agree to the terms and privacy policy.'
  }
...
const handleSubmit = async () => {
  if (!validate()) return
  try {
    await register(form.name, form.email, form.password, form.role, form.institution)
